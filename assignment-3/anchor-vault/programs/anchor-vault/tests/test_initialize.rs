...
fn test_initialize_deposit_withdraw_close() {
    ...
    let (vault_state_pda, state_bump) = Pubkey::find_program_address(&[b"state", user.as_ref()], &anchor_vault::id());

    let (vault_pda, vault_bump) = Pubkey::find_program_address(&[b"vault", vault_state_pda.as_ref()], &anchor_vault::id());
    ...
    let vault_state = anchor_vault::state::VaultState::try_deserialize(&mut vault_state_account.data.as_ref()).unwrap();
    ...
    msg!("Withdraw after deposit: {}", vault_balance_after_withdraw);
